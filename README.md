...
- 현재 기온과 상태는 카드 경계 없이 날씨 배경 위 중앙 Hero에 표시하고,
  서울·수원·세종·인천·부산·대전·대구·광주·울산·제주의 날씨를 반투명 카드 그리드로
  비교합니다.
...
- 초성·완성 전 음절·두벌식 영문 입력을 지원하는 도시 검색과 전체 도시 카드 목록을
  제공합니다. 목록은 처음에는 숨기고 검색을 시작하면 자동으로 펼쳐집니다. 검색창
  아래 `도시 목록` 버튼으로도 부드럽게 펼치거나 다시 접을 수 있으며, 펼친 뒤에는
  일반 문서 스크롤로 메인과 목록을 자유롭게 오갈 수 있습니다.
...
- 상세 화면에서는 이미 확인한 현재 날씨와 관측 시각을 compact 가로 요약으로 줄이고,
  체감온도, 습도, 풍속, 기압, 시정거리, 일출·일몰을 하나의 패널 안에서 6개 행으로
  먼저 표시합니다. 이어서 앞으로 약 24시간의 3시간 단위 예보와 현지 날짜 기준 5일
  예보를 차례로 제공하며, 우상단 새로고침으로 현재 관측과 예보를 함께 갱신합니다.
